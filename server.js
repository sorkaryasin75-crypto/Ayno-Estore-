...
// রেলওয়ে এবং হোস্ট কনফিগারেশন ফিক্স
...
// Security and Static Middleware
app.use(helmet({ contentSecurityPolicy: false }));
app.use(cors());
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

// Static Files Servicing (index.html, CSS, Images)
app.use(express.static(__dirname));
app.use('/uploads', express.static(uploadDir));

// Root URL-এ নিশ্চিতভাবে index.html পাঠানো
app.get('/', (req, res) => {
...
if (botToken) {
    ...
    console.warn('[BOT] TELEGRAM_BOT_TOKEN missing. Admin Bot offline.');
}
...
// Health Check Endpoint (Railway Deploy Verify করার জন্য)
...
if (bot) {
    ...
    bot.onText(/\/(start|admin|dashboard)/, (msg) => {
        if (!isAdmin(msg.from.id)) {
            return bot.sendMessage(msg.chat.id, '⛔ Access Denied: Authorized admins only.');
        }
